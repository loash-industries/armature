...
The combination of permissionless deposits and governance-gated withdrawals creates natural patterns for inter-DAO asset flow:

- *Downward funding:* A parent DAO passes a `SendCoinToDAO` proposal, withdrawing from its treasury and depositing into a Sub-DAO's treasury. The parent's governance authorizes the expenditure; the child's treasury accepts it permissionlessly.

- *Upward revenue:* A Sub-DAO passes a `SendCoin` proposal, withdrawing from its treasury and transferring to the parent DAO's treasury. Revenue generated by departments flows back to the parent automatically.

- *Lateral transfer:* Any DAO can fund any other DAO's treasury directly, enabling donations, grants, contract payments, and alliance dues.

These flows compose naturally with the SubDAO hierarchy, enabling organizations to implement complex budgeting and revenue-sharing schemes without any special-purpose mechanisms. The protocol provides the plumbing; governance provides the policy.
