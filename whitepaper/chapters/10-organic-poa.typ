...
The POA is not a governance tool. It is an organizational _organism_.

This analogy reveals structural properties of the design that a purely technical description obscures. The POA self-amends, self-reproduces, and exhibits lifecycle dynamics analogous to living systems. These are not decorations --- they are consequences of the architecture.
...
This self-referential loop is not a recursion bug --- it is the mechanism by which the POA evolves.
...
POAs reproduce through `CreateSub-POA`. The offspring is a full instance of the same primitive --- same architecture, same governance engine, same proposal system.
...
The parent retains control through the `Sub-POAControl` capability. This control is not absolute domination --- it is a governance relationship that can be exercised or relinquished.
...
The treasury system provides the POA's metabolic infrastructure. Resources flow in through permissionless deposits --- revenue, dues, donations. Resources flow out through governance-authorized withdrawals --- salaries, project funding, infrastructure investment.

In hierarchical structures, metabolism extends across the organizational tree. Parent POAs fund child POAs downward. Child POAs return revenue to parents upward. Peer POAs exchange resources laterally.

The protocol provides the circulatory system. Governance provides the metabolic policy.

== Deliberate Death

A POA can die deliberately through migration. The `SpawnPOA` mechanism creates a successor POA with a new governance model, transfers all assets and capabilities, and destroys the original.
...
#aside[
  ...
  A POA may never reproduce. It may never amend its charter. It may persist indefinitely.
...
Every state change is recorded on-chain --- every vote, every amendment, every treasury flow, every board change. The POA becomes a substrate for history.
...
This permanence transforms the POA from a gameplay tool into a contribution to the game world's culture. Every charter amendment, every treasury allocation, every Sub-POA creation shapes Frontier's trajectory. These decisions remain forever available for inspection by later generations.
