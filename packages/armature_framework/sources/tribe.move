...
public fun create_tribe(
    ...
    // Create parent DAO; vault is returned un-shared so we can populate it.
    let (tribe_dao_id, mut cap_vault) = dao::create_returning_vault(
        &tribe_gov,
        tribe_name,
        tribe_description,
        tribe_image_url,
    ...
    // Create both SubDAOs as owned (un-shared) values.
    let (officer_dao, officer_freeze_cap) = dao::create_subdao(
        &officer_gov,
        officer_name,
        officer_description,
        officer_image_url,
    ...
    // Wire SubDAOControls into the parent vault before sharing it.
    let officer_ctrl = capability_vault::new_subdao_control(officer_dao_id, ctx);
    let officer_ctrl_id = object::id(&officer_ctrl);
    capability_vault::store_cap_init(&mut cap_vault, officer_ctrl);

    let member_ctrl = capability_vault::new_subdao_control(member_dao_id, ctx);
    let member_ctrl_id = object::id(&member_ctrl);
    capability_vault::store_cap_init(&mut cap_vault, member_ctrl);

    // Share everything: vault first, then SubDAOs (controller IDs already known).
